...
const ta1 = new RegExp(
  base + '/tournaments/[0-9]+/teams/([0-9]+)|' + base + '/notifications|' + base + '/config/user$'
);
...
export function validateLocation(history, auth) {
  ...
  if (userLevel === Levels.MasterAdmin) return;
  if (userLevel === Levels.OrgAdmin) return;
  if (userLevel === Levels.Referee) result = isRefereeAllowed(path);
  if (userLevel === Levels.Player)
    result = isPlayerAllowed(path) || isTeamAdminAllowed(path, auth.adminTeamIds);
