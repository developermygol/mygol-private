...
class FacilitySelector extends Component {
    ...
    adaptOptions = (storeFacilities) => {
        ...
        const result = storeFacilities.map(fac => ({ key: fac.id, label: fac.name, value: fac.id}));

        const p = this.props;
        if (p.passProps && p.passProps.wantsEmptyRow) result.unshift({key: 0, value: 0, label: '--'});
        ...
        return (
            <div className='FormField'>
                ...
                <Spinner loading={fcs.loading}>
                    <Select {...field.bind()} onChange={this.onChange} value={this.adaptValue(options, field.value)} isMulti={p.multiSelect} options={options} closeMenuOnSelect={!p.multiSelect} theme={getSelectTheme} />
                </Spinner>
