...
class List extends Component {
    ...
    addEditDeleteButtons = (columns) => {
        ...
        columns.push({
            id: '__actions', label: '', handler: this.getEditDeleteActionsHandler()
        });
    }
        ...
        return (row) => {
            return (
                <div className='ListActionButtons'>
                    {(p.canEdit ? <IconButton onClick={() =>p.editButtonHandler(row)} icon='mode_edit' ><Edit3 size={20} /></IconButton> : null)}
                    {(p.canDelete ? <IconButton onClick={() => (p.confirmDelete ? this.handleDelete(row) : p.deleteHandler(row))} icon='delete'><XCircle  size={20} /></IconButton> : null)}
                </div>
            )
        }
    }
    ...
    getColumns = () => {
        ...
        const fields = this.props.fieldDefinition.filter(f => f.hideInList === undefined || f.hideInList !== true);
    ...
    render() {
        ...
        return (
            <React.Fragment>
                {p.title ? <h2><Loc>{p.title}</Loc></h2> : null }
                {p.hint ? <p className='ListHint'><Loc>{p.hint}</Loc></p> : null }
                {p.showActionBar ? (
                ...
                <MessageBox show={this.state.showDeleteConfirmation} onClose={this.handleDeleteConfirmation} buttons='DeleteCancel' >
                    <p className='ModalHead'><Loc>{p.deleteDialogTitle || 'Confirm deletion'}</Loc></p>
                    <p><Loc>{p.deleteDialogMessage || 'Really delete?'}</Loc> "{this.state.selectedItems ? this.state.selectedItems.name : ''}"?</p>
                </MessageBox>
            </React.Fragment>
        )
    }
