  ...
  render() {
    return (
      <div className="Card Hero">
        <h3>
          <Loc>Sanctions.AutoCycleConfig</Loc>
        </h3>
        <div className="Content">
          <p className="Hint">
            <Loc>Sanctions.AutoCycleConfig.Hint</Loc>
          </p>
          <AutoCyclesTable callbacks={this.props.callbacks} cycles={this.props.cycles} />
          <div className="FormField">
            <label className="Label">
              <Loc>Sanctions.CyclesAcrossStages</Loc>
            </label>
            <Slider
              className="with-label"
              onChange={this.props.callbacks.updateCyclesAcrossStages}
              checked={this.props.cyclesAcrossStages}
              label={Localize('Sanctions.CyclesAcrossStages.Hint')}
            />
          </div>
        </div>
      </div>
...
class CardCombinationsConfig extends Component {
  render() {
    return (
      <div className="Card Hero">
        <h3>
          <Loc>Sanctions.AutoCardConfig</Loc>
        </h3>
        <div className="Content">
          <p className="Hint">
            <Loc>Sanctions.AutoCardConfig.Hint</Loc>
          </p>
...
class AutoSanctionsConfig extends Component {
  ...
  @action updateCyclesAcrossStages = e => {
    const parsed = this.props.store.autosanctions.parsed;
    parsed.cyclesAcrossStages = e.target.checked;
  };
  ...
  callbacks = {
    ...
    updateCyclesAcrossStages: this.updateCyclesAcrossStages,
  };
  ...
  render() {
    ...
    const cyclesAcrossStages = parsed && parsed.cyclesAcrossStages;

    return (
      <Fragment>
        <CardCombinationsConfig callbacks={this.callbacks} cards={cards} />
        <CycleConfig callbacks={this.callbacks} cycles={cycles} cyclesAcrossStages={cyclesAcrossStages} />

        <div className="">
          <button className="Button" onClick={this.showJsonHandler}>
            <Loc>Payment.ShowEditJsonForm</Loc>
          </button>
          <button className="Button Active" onClick={this.saveHandler}>
            <Loc>Sanctions.SaveConfig</Loc>
          </button>
        </div>
