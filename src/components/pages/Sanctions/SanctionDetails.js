            ...
            <div className='Sanctions Card Hero'>
                <h3>
                    ...
                    <AccessLimit allowOrgAdmin>
                        <button className='Button Second Right' onClick={() => p.editButtonHandler(sanction)}><Loc>Edit</Loc></button>
                    </AccessLimit>
                </h3>
        ...
        return (
            <div className='SubDetail'>
                ...
                <div className='Horizontal'>
                    ...
                    <AccessLimit allowOrgAdmin><p className='Date'>{al.visible ? Localize('Visible') : Localize('NotVisible')}</p></AccessLimit>
                    <div className='Actions'>
                        {canEdit ? <button className='Button Second' onClick={p.onAllegationEdited}><Loc>Edit</Loc></button> : null}
                        {isOrgAdmin ? <button className='Button Second' onClick={p.onAllegationRemoved}><Loc>Delete</Loc></button> : null}
                    </div>
                </div>
    ...
    render() {
        ...
        return (
            <div className='Card Hero'>
                ...
                <div className='Content'>
                    {p.allegations && p.allegations.map(al => <SanctionAllegation key={al.id} allegation={al} onAllegationEdited={() => p.onAllegationEdited(al)} onAllegationRemoved={() => p.onAllegationRemoved(al)} />)}
                    <div className='Actions'>
                        <button className='Button Second' onClick={p.onAllegationAdded}><Loc>Add new allegation</Loc></button>
                    </div>
                </div>
            </div>
        )
...
class SanctionMatches extends Component {
    render() {
        ...
        return (
            <div className='Card Hero'>
                <h3><Loc>Sanction.Matches</Loc></h3>
...
class SanctionDetails extends Component {
    ...
    allegationAddedHandler = (allegation) => {
        ...
        this.props.store.sanctions.addAllegation(allegation)
    ...
    allegationEditedHandler = (allegation) => {
        ...
        this.props.store.sanctions.editAllegation(allegation)
            .then(res => this.setState({}));
    }
    ...
    allegationDeletedHandler = (allegation) => {
        ...
        this.props.store.sanctions.removeAllegation(allegation)
            .then(res => this.setState({}));
    }
            ...
            <Spinner loading={store.loading}>
                {sanction ? 
                    <Fragment>
                        ...
                        <SanctionAllegations allegations={sanction.allegations} onAllegationAdded={this.addAllegationHandler} onAllegationEdited={this.editAllegationHandler} onAllegationRemoved={this.removeAllegationHandler}   />
