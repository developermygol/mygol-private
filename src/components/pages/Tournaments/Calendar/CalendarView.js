...
import { getSelectOptionsFromFixedValues, getSelectOptionsFromTable } from '../../../common/FormsMobx/EditRenderHandlers';
...
class CalendarView extends Component {
    ...
    createSaveHandler = (data) => {
        ...
        return this.props.store.matches.createMatch(data)
            .then(res => {
                if (res) goBack(this);
            })
    }
    ...
    editSaveHandler = (data) => {
        ...
        return this.props.store.matches.editMatch(data)
            .then(res => {
                if (res) goBack(this);
            })
    }
    ...
    getAdditionalButtons = () => {
        ...
        if (p.match.params.idGroup) 
            return <button className='Button' onClick={this.deleteCalendarHandler}><Loc>Delete calendar</Loc></button>

        return <button className='Button' onClick={this.addDayHandler}><Loc>Add playday</Loc></button>
    }
            ...
            <div>
                <CrudForm
                    ...
                    fieldDefinition={[
                        ...
                        { fieldName: 'startTime', localizedLabel: 'Match.StartTime', hint: 'Match.StartTime.Hint', editRenderType: 'text', rules: '' },
                        { fieldName: 'duration', localizedLabel: 'Match.Duration', hint: 'Duration.Hint', editRenderType: 'text', rules: 'integer' },
                        { fieldName: 'status', localizedLabel: 'Match.Status', hint: '', editRenderType: 'select', selectOptions: getSelectOptionsFromFixedValues('MatchStatus', 1, 5), hideInAdd: true },
                        { fieldName: 'idStage', localizedLabel: 'Match.Stage', hint: '', editRenderType: 'select', selectOptions: this.getStageSelectOptions(stages), hideInEdit: true, rules: 'integer|required', onChange: this.stageChanged },
                        { fieldName: 'idGroup', localizedLabel: 'Match.Group', hint: '', editRenderType: 'select', selectOptions: this.getGroupSelectOptions(groups), hideInEdit: true, rules: 'integer|required' },
                        { fieldName: 'idDay', localizedLabel: 'Match.PlayDay', hint: '', editRenderType: 'select', selectOptions: this.getPlayDaysSelectOptions(p.value), rules: 'integer|required', hideInEdit: true },
                        { fieldName: 'idField', localizedLabel: 'Match.Field', hint: '', editRenderType: 'facilitySelector', rules: '' },
                        { fieldName: 'idHomeTeam', localizedLabel: 'Match.HomeTeam', hint: '', editRenderType: 'select', selectOptions: teamsSelectOptions },
                        { fieldName: 'idVisitorTeam', localizedLabel: 'Match.VisitorTeam', hint: '', editRenderType: 'select', selectOptions: teamsSelectOptions },
                        { fieldName: 'homeTeamDescription', localizedLabel: 'Match.HomeTeamDescription', hint: 'Match.HomeTeamDescription.Hint', editRenderType: 'text' },
                        { fieldName: 'visitorTeamDescription', localizedLabel: 'Match.VisitorTeamDescription', hint: 'Match.VisitorTeamDescription.Hint', editRenderType: 'text' },
                        { fieldName: 'videoUrl', localizedLabel: 'Match.VideoUrl', hint: 'Match.VideoUrl.Hint', editRenderType: 'text', rules: '' },
                        { fieldName: 'comments', localizedLabel: 'Match.Comments', hint: 'Match.Comments.Hint', editRenderType: 'textarea', rules: '', hideInAdd: true },
                    ]}
                />
                <MessageBox show={this.showDeleteCalendarConfirmation} onClose={this.calendarDeletedHandler} buttons='DeleteCancel' >
                    <p className='ModalHead'><Loc>{'Confirm deletion'}</Loc></p>
                    <p><Loc>Confirm delete calendar</Loc></p>
                </MessageBox>
                <MessageBox show={this.showDeletePlayDayConfirmation} onClose={this.dayDeletedHandler} buttons='DeleteCancel' >
                    <p className='ModalHead'><Loc>{'Confirm deletion'}</Loc></p>
                    <p><Loc>PlayDay.ConfirmDelete</Loc></p>
                </MessageBox>
