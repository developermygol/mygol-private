...
class EventList extends Component {
    ...
    getTypeElement = (event) => {
        const { type } = event;
        return <span className={'EventType Type' + type}>{Localize('MatchEventType' + type)}{event.type === 1002 && (' (' + event.intData1 + ')')}</span>;
    }

    getTypeElementWithTime = (type, minute) => {
        return <span className={'EventType Single Type' + type}>{Localize('MatchEventType' + type)} ({minute}")</span>;
    }
            ...
            <Fragment>
                ...
                {(!match || !match.events || match.events.length === 0) ?
                    <Fragment>
                        ...
                        <button className='Button Center' onClick={this.addButtonHandler}><Loc>{this.props.addMessage}</Loc></button>
                    </Fragment>
                    :
                    <Fragment>
                        <button className='Button Center' onClick={this.addButtonHandler}><Loc>{this.props.addMessage}</Loc></button>
                                        ...
                                        {ev.idTeam === 0 ?
                                            <Fragment>
                                                <div className='Wide' colSpan={5}>
                                                    ...
                                                    <IconButton onClick={() => this.deleteEventHandler(ev)} icon='delete' ><XCircle size={15}/></IconButton>
                                                </div>
                                                
                                            </Fragment>
                                            :
                                            <Fragment>
                                                <div className='EntryBox Left Delete'>{first ? <IconButton onClick={() => this.deleteEventHandler(ev)} icon='delete' ><XCircle size={20}/></IconButton> : null}</div>
                                                <div className='EntryBox Left Player'>{first ? this.getPlayerRender(match, ev.idTeam, ev.idPlayer, first) : null}</div>
                                                ...
                                                <div className='EntryBox Right Player'>{first ? ' ' : this.getPlayerRender(match, ev.idTeam, ev.idPlayer, first)}</div>
                                                <div className='EntryBox Right Delete'>{first ? ' ' : <IconButton onClick={() => this.deleteEventHandler(ev)} icon='delete' ><XCircle size={20}/></IconButton>}</div>
                                            </Fragment>
                                        }
                                    </div>
                                )
