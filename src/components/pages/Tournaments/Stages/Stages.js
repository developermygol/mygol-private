...
class Stages extends Component {
  ...
  deleteStageHandler = stage => {
    const msg = (
      <p>
        <Loc>Really delete?</Loc> {stage.name}?
      </p>
    );
  ...
  deleteGroupHandler = group => {
    const msg = (
      <p>
        <Loc>Really delete?</Loc> {group.name}?
      </p>
    );
  ...
  teamCanDrop = (team, group) => {
    const teamAlready = this.props.store.teamGroups.all.filter(
      tg => tg.idStage === group.idStage && tg.idTeam === team.id
    );
  ...
  render() {
    ...
    return (
      <div>
        {readOnly ? (
          <InfoBox>
            <Loc>Stages.TournamentAlreadyStarted</Loc>
          </InfoBox>
        ) : null}
        <div className="Flex">
          <div className="Stages">
            ...
            {readOnly ? null : (
              <button className="Button Second" onClick={this.addStageHandler}>
                <Loc>Add stage</Loc>
              </button>
            )}
            <StagesTable
