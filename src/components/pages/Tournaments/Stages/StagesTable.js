        ...
        return (
            <Fragment>
                <StageRow 
                    ...
                    {...p}
                />
                {this.getStageGroups().map(group => <Group
                                                key={group.id} 
                                                allowChanges={allowChanges}
        ...
        return (
            <tr>
                <td colSpan={8} className='Level1'>
                    <span className='StageName'>{stage.name}</span> <span className=''><Loc>{'StageType' + stage.type}</Loc> </span>
                    <span className={'StageStatus StageStatus' + stage.status}> <Loc>{'StageStatus' + stage.status}</Loc></span>
                    {p.edit ?
                    <span className='Actions'>
                        {editable && !p.readOnly ? <button className='Button Third' onClick={() => cbs.addGroup(stage)}><Loc>Add group</Loc></button> : null }
                        {/* {!active ? <button className='Button Third' onClick={() => cbs.setActiveStage(stage)}><Loc>Set as active</Loc></button> : null } */}
                        <IconButton className='Button Third' onClick={() => cbs.editStage(stage)}><Edit3 size={20} /></IconButton>
                        {!p.readOnly ? <IconButton className='Button Third' onClick={() => cbs.deleteStage(stage)}><XCircle  size={20} /></IconButton> : null }
                    </span>
                    : null }
                </td>                    
            </tr>
...
class GroupRow extends Component {
    render() {
        ...
        return (
                <tr>
                    <td className='Level2'><span className='GroupName'>{group.name}</span> <span className=''>({group.numTeams + ' ' + Localize('teams')})</span></td>
                    {p.showResults ? 
                    <Fragment>
                        <td className='Number'><Loc>TournamentPointsShort</Loc></td>
                        <td className='Number GamesWon '><Loc>GamesWonShort</Loc></td>
                        <td className='Number GamesDraw'><Loc>GamesDrawShort</Loc></td>
                        <td className='Number GamesLost'><Loc>GamesLostShort</Loc></td>
                        <td className='Number Points   '><Loc>PointsShort</Loc></td>
                        <td className='Number PointsAgainst'><Loc>PointsAgainstShort</Loc></td>
                    </Fragment>
                    : null }
                    {p.edit ? 
                    <td colSpan={ p.showResults ? 1 : 7 }>
                        <span className='Actions'>
                            <button className='Button Third' onClick={() => cbs.editGroupCalendar(group)}><Loc>Calendar</Loc></button>
                            <IconButton className='Button Third' onClick={() => cbs.editGroup(group)}><Edit3 size={20} /></IconButton>
                            {!p.readOnly ? <IconButton className='Button Third' onClick={() => cbs.deleteGroup(group)}><XCircle  size={20} /></IconButton> : null }
                        </span>
                    </td>
                    : null }
                </tr>
        ...
        return (
            <Fragment>
                <tr>
                    {teamGroup ? 
                        <Fragment>
                            <td className='Level3' colSpan={p.showResults ? 1 : 7}>
                                ...
                                <IconButton className='Button Third' onClick={() => p.callbacks.deleteTeamGroup(teamGroup)}><XCircle  size={20} /></IconButton>
                            </td>
                            {p.showResults && sd ? 
                                <Fragment>
                    ...
                    : 
                        <Fragment>
                            <td className='Level3' colSpan={7}><TeamDropContainer sequenceOrder={p.sequenceOrder} {...p} /></td>
                        </Fragment>
                    }
                </tr>
            </Fragment>
