...
class CompetitionTeams extends Component {
  ...
  getTeamLogo(t) {
    return this.getTeamLink(
      ...
      <img
        alt=""
        src={getUploadsIcon(t.logoImgUrl, t.id)}
        className="TeamLogo"
      />
    );
  }
    ...
    return (
      <div>
        <CrudForm
          ...
          addAction={(data) =>
            teams.actions.create(data, teams.getCreateUrl(tId))
          }
          editAction={(data) => teams.actions.edit(data)}
          deleteAction={(data) =>
            teams.actions.remove(data, teams.getDeleteUrl(tId))
          }
          ...
          fieldDefinition={[
            ...
            //{ fieldName: 'prefTime', localizedLabel: 'Preferred time', hint: 'Preferred time.hint', editRenderType: 'time', hideInList: true },
            {
              ...
              listRenderHandler: (r) =>
                listArrayLookup(teamStates, r, "status"),
